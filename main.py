...
"""
        ...
        is_relevant = format_result['is_relevant'] == '1'
        ...
        if not is_relevant:
            logger.info(
                f"在 {account_type}: {account_id} 上发现有更新的内容，但内容与需要关注的主题无关")
            logger.debug(f"不相关内容: {content[:100]}..." if len(content) > 100 else content)
            return result

        logger.info(f"在 {account_type}: {account_id} 上发现相关内容，准备发送通知")
...
origin: [{post.url}]({post.url})"""
        ...
        # 如果启用了自动回复，尝试回复
        if enable_auto_reply and account.get('enableAutoReply', False):
            try:
        ...
        if save_to_db:
            try:
                ...
                db_result = AnalysisResult(
                    ...
                    post_time=post_time,
                    content=content,
                    analysis=format_result['analytical_briefing'],
